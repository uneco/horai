...
class Horai
  ...
  def self.register_filters
    ...
    filter /(\d+)時(半)?/, :absolute do |text, matches, date|
      ...
      date += datetime_delta(date, :second, 0)
      if date.hour <= 12 && text =~ /午後|ごご|夜|よる|(?<![a-z])pm(?![a-z])/i
        date += 12.hour
      end
      date
    end
    ...
    filter /一昨[々昨]日|さきおと[とつ]い|いっさくさくじつ/, :absolute do |text, matches, date|
      relative_time_fixer(year: 0, month: 0, day: -3).call(text, matches, date)
    end

    filter /一昨日|おと[とつ]い|いっさくじつ/, :absolute do |text, matches, date|
      relative_time_fixer(year: 0, month: 0, day: -2).call(text, matches, date)
    end

    filter /昨日|きのう|さくじつ/, :absolute do |text, matches, date|
      relative_time_fixer(year: 0, month: 0, day: -1).call(text, matches, date)
    end

    filter /今日|本日|きょう|ほんじつ/, :absolute do |text, matches, date|
      relative_time_fixer(year: 0, month: 0, day: 0).call(text, matches, date)
    end
    ...
    filter /[先前][々先前]{3}月|せんせんせんげつ/, :absolute do |text, matches, date|
      relative_time_fixer(year: 0, month: -3).call(text, matches, date)
    end

    filter /[先前][々先前]月|せんせんげつ/, :absolute do |text, matches, date|
      relative_time_fixer(year: 0, month: -2).call(text, matches, date)
    end

    filter /先月|前月|せんげつ/, :absolute do |text, matches, date|
      relative_time_fixer(year: 0, month: -1).call(text, matches, date)
    end

    filter /今月|こんげつ/, :absolute do |text, matches, date|
      relative_time_fixer(year: 0, month: 0).call(text, matches, date)
    end
    ...
    filter /一昨[々昨]年|さきおととし|いっさくさくねん/, :absolute do |text, matches, date|
      relative_time_fixer(year: -3).call(text, matches, date)
    end

    filter /一昨年|おととし|いっさくねん/, :absolute do |text, matches, date|
      relative_time_fixer(year: -2).call(text, matches, date)
    end

    filter /去年|昨年|きょねん|さくねん/, :absolute do |text, matches, date|
      relative_time_fixer(year: -1).call(text, matches, date)
    end

    filter /今年|本年|ことし|ほんねん/, :absolute do |text, matches, date|
      relative_time_fixer(year: 0).call(text, matches, date)
    end
